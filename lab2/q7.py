 ...
 with tf.Session() as sess:
   ...
   for ii in range(EPOCHS+1):
        ...
        if ii % PRINT_STEP == 0:
         ...
         print ('iteration :' + str (ii) + ',' + 'training cost:' + str (cost)+','+'method:'+str(method))
       ...
       # print the fianl result in interations equal to 5000
         if ii == 1000:
           ...
           worksheet.write (0, col, 'Optimizermethod')
           worksheet.write (0, col + 1 , 'prediction_mse')
           worksheet.write (0, col + 2, 'prediction_rmse')
           worksheet.write (0, col + 3, 'prediction_mae')
           worksheet.write (0, col + 4, 'prediction_mape')
           worksheet.write (0, col + 5, 'cost')
           ...
           worksheet.write (row + 1, col, str (method))
           worksheet.write (row+1, col + 1 , str (prediction_mse))
           worksheet.write (row+1, col + 2, str (prediction_rmse))
           worksheet.write (row+1, col + 3, str (prediction_mae))
           worksheet.write (row+1, col + 4, str (prediction_mape))
           worksheet.write (row + 1, col + 5, str (cost))
...
for i in range(5):
    if i==0:
       ...
       calculate(0)
    if i==1 :
       train_op = tf.train.RMSPropOptimizer (learning_rate, 0.2).minimize (cost_func)
       calculate(1)
    if i==2 :
       train_op = tf.train.GradientDescentOptimizer (learning_rate).minimize (cost_func)
       calculate(2)
    if i==3 :
       train_op = tf.train.AdagradOptimizer(learning_rate).minimize (cost_func)
       calculate(3)
    if i==4 :
       train_op = tf.train.FtrlOptimizer(learning_rate).minimize (cost_func)
       calculate(4)
